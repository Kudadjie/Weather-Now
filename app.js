const weatherApp =  {
		...
		tempUnitF: document.querySelector(".temp-f"),
		tempUnitC: document.querySelector(".temp-c"),
			...
			for (let j in this.backgrounds[i].regex) {
			  if (this.backgrounds[i].regex[j].test(conditiontext)){
				...
				if (this.previousBackground) {
				  ...
				  this.previousBackground = this.backgrounds[i].cssClassToToggle[day][0];
    ...
    changeTempUnit(){
      [this.optionTempUnit.textContent, this.currentTempUnit.textContent] = [this.currentTempUnit.textContent,this.optionTempUnit.textContent]
      if(this.tempUnit == 'C'){
		...
		changeTimeFormat(string) {
		  if (/AM/i.test(string) || /PM/i.test(string)) {
			let time = string.match(/(^\d+:\d+).+\s(\w+$)/);
			return `${time[1]} ${time[2]}`;
		  } else {
			const suffixs = ["AM", "PM"];
			let suffix;
			let hours = Number(String(string).match(/^\d\d/));
	  
			let seconds = String(string).match(/\d\d$/);
			if (hours < 12) {
			  suffix = suffixs[0];
			}
			if (hours > 12) {
		...
		changeDateformat(string) {
		  ...
		  const suffixs = ["st", "nd", "rd", "th"];
		  let day = Number(String(string).slice(8));
		  let month = months[Number(String(string).slice(5, 7)) - 1];
		  let suffix;
		...
		closeModal(type) {
		  type.classList.add("close-modal");
		},
	  
		openModal(type) {
		  type.classList.remove("close-modal");
		},
...
//TO DO
/*
Temp unit switch functionality
...
weatherApp.optionTempUnit.addEventListener("click", ()=>{
  weatherApp.changeTempUnit(dataDump)
})
